...
local function GetSmoothedAngularVelocity(pEntity)
    ...
    if not samples or #samples < 3 then return 0 end
    ...
    if #ang_vels == 0 then return 0 end
...
local function TryPlayerMove(origin, velocity, frametime, mins, maxs, shouldHitEntity, surface_friction)
    ...
    for bumpcount = 0, 3 do
        if velocity:LengthSqr() < 0.0001 then break end
        ...
        if trace.fraction >= 0.99 then break end
...
local function StepMove(origin, velocity, frametime, mins, maxs, shouldHitEntity, surface_friction, step_size,
                        is_on_ground)
